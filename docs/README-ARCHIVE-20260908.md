...
## Personal OS update: Life, Create, Earn

U1 OS now gives **My Day**, **Digital Studio**, **Income**, **Crypto** and
**Trading** visible destinations inside the main shell. The rail is organised
into Life, Create, Earn and Control, with less-used tools under More workspaces.

| Workspace | Included in this source update |
| --- | --- |
| My Day | Actual local calendar events, open tasks and projects, plus daily-life shortcuts |
| Digital Studio | Eleven printable templates, course workbooks, product launch kits and original SVG covers |
| Income | Offer planning, creation of local income projects and access to the actual-entry manual ledger |
| Crypto and Trading | Restored research views, source-labelled snapshots and published market schedules |
| Safety Centre | Opt-in passphrase setup, focused-window dead-man shortcut, optional check-in timer and offline access locking |

The new Studio exports real local PDFs. AI image generation and automatic Canva
sync are **not connected** by this update. Generated covers are vector artwork,
not AI bitmap images. No example revenue, automatic publishing, autonomous trades
or shared cross-provider allowance is claimed.

The safety system locks U1 OS access and new API requests, not the Mac itself.
Existing jobs, streams, exchange orders and other applications are not cancelled.
It must be configured by its owner before it is armed. Reconnection does not
unlock it automatically.

Read the [personal OS guide](docs/PERSONAL-OS.md) and
[safety setup and limitations](docs/SAFETY-LOCK.md). This increment remains a
source implementation requiring functional and visual validation; dated test
results and preview images elsewhere in this repository describe earlier builds.
