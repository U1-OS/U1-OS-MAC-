...
def main():
    ...
    # 99. Wave 9: Sensory Layer (Motion, Audio Synth & Notification Centre)
    print(f"\n{INFO} 99. Subsystem: Sensory Layer // Motion, Audio Synth & Notification Centre:")
    s_fx_js, fx_js = get_text("/js/fx.js")
    log_test("Sensory Layer Client Asset Served (/js/fx.js)", s_fx_js == 200 and len(fx_js) > 4000, f"HTTP {s_fx_js} | {len(fx_js)} bytes delivered")
    s_fx_css, fx_css = get_text("/css/fx.css")
    log_test("Motion & Notification Stylesheet Served (/css/fx.css)", s_fx_css == 200 and "@keyframes fxToastIn" in fx_css, f"HTTP {s_fx_css} | {fx_css.count('@keyframes')} keyframe animations compiled")
    log_test("Sensory Layer Assets Linked In Application Shell", '/js/fx.js' in index_html and '/css/fx.css' in index_html, "fx.js and fx.css mounted in index.html document head/body")
    log_test("Global showNotification() Entry Point Defined", "window.showNotification" in fx_js, "Global notification entry point exported (resolves all legacy app.js call sites)")
    log_test("Web Audio Synthesizer Voice Pack Complete", all(v in fx_js for v in ["click:", "tick:", "nav:", "success:", "error:", "warn:", "trade:", "execute:", "cash:", "boot:", "signal:", "notify:"]), "12+ synthesized interface voices registered (zero binary audio assets)")
    log_test("Audio Engine Master Gain, Mute & Persistence Controls", "setVolume" in fx_js and "setEnabled" in fx_js and "u1os.fx.volume" in fx_js, "Master gain node, mute toggle and localStorage persistence active")
    log_test("Application Controller Audio Voices Resolve (tick/execute/error)", "tick:" in app_js and "execute:" in app_js and "error:" in app_js, "Previously-undefined AudioFeedback voices now implemented in app.js")
    log_test("Notification Centre Panel & Unread Badge Construction", "notifPanel" in fx_js and "notifBadge" in fx_js and "btnNotifCenter" in fx_js, "Bell trigger, unread badge and history panel injected into top bezel")
    log_test("Notification Feed Persistence & Capacity Ceiling", "u1os.fx.feed" in fx_js and "FEED_CAP" in fx_js, "Signal history persisted across reloads with a bounded ring")
    log_test("Toast Stack Severity Classification & Auto-Inference", "inferType" in fx_js and "t-success" in fx_css and "t-error" in fx_css, "Severity inferred from message wording; 5 colour-coded toast variants styled")
    log_test("Toast Message Rendering Is Injection-Safe", "textContent" in fx_js and "escapeHtml" in fx_js, "Event payloads rendered as text, never as markup")
    log_test("Native Desktop Notification Bridge", "Notification.permission" in fx_js and "requestPermission" in fx_js, "Browser/macOS desktop notification bridge wired with permission negotiation")
    log_test("Motion Engine Ripple, Reveal, Count-Up & Value Flash", all(v in fx_js for v in ["ripple", "revealAll", "countUp", "flash", "sectionEnter", "bootSweep"]), "6 motion primitives exposed on U1FX.motion")
    log_test("Live SSE Events Mirrored Into Notification Centre", "Wave 9: mirror every live SSE event" in app_js, "SSE service events feed the persistent notification history")
